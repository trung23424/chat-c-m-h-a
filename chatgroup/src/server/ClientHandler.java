        ...
        try {
            ...
            while ((msg = in.readLine()) != null) {
                ...
                // 👉 Xử lý gửi RSA message theo nhóm phòng
                    ...
                    String clearText = msg.substring(13).trim();

                    for (ClientHandler client : Server.clientMap.values()) {
                        if (client != this && client.currentRoom.equals(this.currentRoom)) {
                            PublicKey recipientKey = Server.publicKeyMap.get(client.username);
                            if (recipientKey == null) continue;

                            try {
                                byte[] encrypted = security.EncryptionUtil.encrypt(clearText.getBytes(), recipientKey);
                                String base64 = Base64.getEncoder().encodeToString(encrypted);
                                System.out.println("🔐 Gửi đến " + client.username + ": /rsamsg " + this.username + " " + base64);
